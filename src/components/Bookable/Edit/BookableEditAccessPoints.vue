 ...
 * by the provider - one per unit the booking books. There is no number per
 * access point: the bookable's `amount` (Stückzahl, pricing tab) is the
 * capacity the concurrent bookings are counted against, and the provider is
 * asked live whether it has the compartments. A map a bookable saved as
 * `accessPointAmounts` before that field was retired is passed through
 * untouched here and dropped by `ApiBookablesService` on the way out.
 *
...
export default {
  ...
  methods: {
     ...
     * compartment per unit the booking books (`bookableItem.amount`) - never
     * the bookable's `amount`, which is the capacity the concurrent bookings
     * are counted against. A door is not handed out at all; it is shared for
     * the booking period.
     */
    grantText(row) {
      return row.isLocker
