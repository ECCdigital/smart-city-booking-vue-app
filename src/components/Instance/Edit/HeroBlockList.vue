<template>
  <div>
    <p class="text--secondary text-body-2 mb-3">
    ...
    <v-card v-if="groups.length > 0" outlined class="mb-3">
      <div v-for="group in groups" :key="group.zone" class="hero-block-group">
        <div
          class="hero-block-group__label text-caption text--secondary px-3 py-1"
        >
          {{ group.label }}
        </div>
        ...
        >
          <div
            ...
            :key="block.id"
            :data-id="block.id"
            class="hero-block-row d-flex align-center px-2 py-1"
            :class="{
    ...
    <p v-else class="text--secondary text-body-2 mb-3">Noch keine Blöcke.</p>
    ...
    <p v-if="atMaxBlocks" class="text--secondary text-caption mt-1 mb-0">
...
/**
 * The „Blöcke“ section of the Hero Editor: the Blocks in nine groups by Zone,
 * the ⋮ menu of a row and „Block hinzufügen“.
 *
...
/* The separator and the two tints are ink on a light sheet; on a dark one they
   have to be light, or the group and the selection disappear. */
