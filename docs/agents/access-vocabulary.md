...
| Zugangsfenster (Access Window)                | `accessFrom` / `accessTo`, `isBeforeAccessWindow()`, `isAfterAccessWindow()`, `isWithinAccessWindow()` | The span in which a booking's Zugänge may be operated: the booking period plus the Pufferzeit. Viewing is allowed outside it; operating is not, except through the Admin Override.            |
| Admin Override                                | `closeBlockReason()` / `statusBlockReason()` in `BookingAccessPoints.vue`                              | Whoever may control the booking may close a door and read its status after the Zugangsfenster has ended — never open, never before it starts. The backend marks such rows `windowOverridden`. |
| Lock Busy                                     | `isLockBusyError()`                                                                                    | The provider refused a command because the lock is still carrying out the previous one (backend HTTP 423, `code: "lock_busy"`). Shown as the tile alert; no retry, no Cooldown in the admin.  |
