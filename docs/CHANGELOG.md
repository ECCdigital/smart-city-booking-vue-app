...
-   BFF: a failed CSRF check answers `419` instead of `403` (`bff/src/csrf.js`, all three rejection paths; body unchanged). A `403` from the BFF was indistinguishable from the backend's `ForbiddenError`, so an expired CSRF session showed "Sie haben keine Berechtigung für diese Aktion." `getApiErrorMessage` translates `419` to a session hint instead ("Bitte laden Sie die Seite neu"). Nothing on the pass-through path changed — the proxy and both auth transports hand the status on untouched. The BFF has no test setup; its side is covered by the UI specs and by `bff/README.md`. **Upgrade note:** UI and BFF have to be deployed together; a new UI in front of an old BFF falls back on the 403 branch and shows the generic denial
