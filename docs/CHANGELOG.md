...
-   Hero Editor: „Darstellung“ opens with a preview tile — the Block painted as it will paint, on a strip of the Draft's **own** Background rather than a fixture photo, so a translucent Panel has something to be translucent against and an author reads what „Deckkraft 40“ means before looking at the frames. It paints what the section sets and nothing else: the Panel's `color`, `opacity`, `radius` and `blur` on the Block's own box — the frosting omitted at 100 %, where it would paint nothing, so a Panel at 0 % is pure frosting and one at 100 % an opaque fill — the Block's `align` inside that box, and the Block's own text `size` and `color`, with „Standard“ as the ink of the light mode and the two branded tokens out of the instance's real `branding.theme.colors`. A Block without a Panel shows as it is, with no phantom surface. The strip carries all three Background families: „Farbe“ as the colour itself, „Bild“ as the image under its overlay (the overlay colour at full opacity standing in while the image is still to be chosen, as the contract's own placeholder does), and „Muster“ as a neutral surface that **names** the pattern — „Nordlicht“, „Polygone“ — because the five generated patterns live in the storefront's generator and inventing a second one here is exactly the drift to avoid. This is the one place in the admin that repaints what the storefront paints, so its scope is drawn twice: the tile leaves the Zone, the Offset and the Layer to the two real frames and says so under itself („Zone, Versatz und Ebene zeigt die Live-Vorschau“), and it holds **no state of its own** — every value is a computed of the Block, the Background and the branding it is handed, so it cannot show a value the frames do not receive. It paints the mobile tree throughout, which is where the six text steps are the scale's own rem values and „Automatisch“ centres because there are no columns. The derivations live in `src/utils/heroTileStyle.js` and read the Background through `heroBackground.js`, so a Background that predates a default reads here exactly as it reads in „Hintergrund“; the tile itself is `HeroBlockTile.vue`, and „Darstellung“'s group order changed with it — the tile, then the Panel group, „Lage“ and „Feinabstimmung“
-   Hero Editor: a „Formatierter Text“ Block stops being fixed at body copy. Its own „Schriftgröße“ on the six steps (Sehr klein, Klein, Normal, Groß, Sehr groß, Riesig) now stands beside the colour control and above the editor — the two are what a run of words inherits while it carries no class of its own, so the author reads them first — and a Block that stores none reads as „Normal“, the default the contract fills in on save. It is the same select and the same `HeroColorField` a text Block has, not a second pair. Below them the editor runs with `sizes`, `colors`, `paragraphAlign` and the instance's real `branding.theme.colors` beside the `links` and `maxLength` of 10 000 it already had, still one instance per locale view: the two-row leiste „Zeichen“ / „Absatz“ is what the Hero Editor mounts from now on, and the character counter keeps counting and the link button keeps working inside it. Marked-up rich text loads back out of the editor byte-identical — an aligned paragraph, a `hero-size-*`/`hero-color-*` span, a custom hex with its `data-color` and the admin-only `style`, a link and a list all survive — with the normaliser applying no repair and the form emitting no patch, and switching the language toggle keeps the marks of both locale views. The characterisation that a rich-text Block has no „Schriftgröße“ changed with it
-   Hero Editor: „Lage“, between the Panel group and „Feinabstimmung“ — a 3×3 nudge pad for the Block's Offset, the eight directions as arrow buttons carrying their German words as `title` and `aria-label`, one click one step of 0.5 rem, clamped to −3…3 rem so six steps each way, with a direction that has run out of grid disabled and each axis clamped on its own, so a diagonal at the right edge still moves downwards. The middle cell resets to `{ x: 0, y: 0 }` and is disabled while the Block has not moved. Beside the pad a readout that counts steps rather than rem — „Kein Versatz“, or „4 nach rechts · 1 nach oben (Schritte)“ for `{ x: 2, y: -0.5 }` — and the switch „Im Vordergrund“, which writes `layer` as `front` and `back`. The pad cannot produce a value the contract refuses, so it takes no local rule of its own; a backend `400` at `heroLayout.blocks[2].offset.x` marks the Block's row and is named „Lage“, and one at `…layer` „Im Vordergrund“. The step arithmetic, the nine cells and the readout live in `src/utils/heroOffset.js`. Nothing in the Preview Report changes: the overlap a nudged Block makes is suppressed by the storefront, which holds the rendered layout
-   Hero Editor: „Darstellung“ now leads with the Block's own look and demotes what it inherits. The Panel group comes first — the switch „Fläche hinter dem Block“, which writes `null` when it is off and the „Glas“ defaults when it is on, with the chip „Glas“ beside it that rewrites all four keys at once from any state, an off Panel included. While the switch is on it opens: „Farbe“ as the Panel's own row of colour dots (Weiß, Schwarz, Primärfarbe, Sekundärfarbe, Eigene…, the last opening the hex picker the Block's colour field already uses, without alpha), the slider „Deckkraft“ from 0 to 100 with a per-cent readout, „Ecken“ as five drawn corner steps (Kein, Klein, Mittel, Groß, Rund) and the switch „Hintergrund weichzeichnen“ — each control writing exactly its own key and leaving the other three alone. The four controls a Block inherits move unchanged into a closed „Feinabstimmung“: „Ausrichtung“, „Außenabstand“, „Innenabstand“ and „Breite“. „Ausrichtung“ is new there and applies to every Block type, images included, with the hint „Automatisch folgt der Spalte der Position.“ and a second one, „Wirkt erst ab einer festen Breite.“, while „Breite“ is „Automatisch“, because a Block that shrinks to fit has nothing to align inside. The switch „Halbtransparente Fläche hinter dem Block“ is gone; the Panel group replaces it, and its two specs changed with it. The Panel's „Eigene…“ colour takes the same hex rule the Block's colour field has, on every Block type, so an invalid one marks the row and disables „Speichern“; a backend `400` at `heroLayout.blocks[2].panel.opacity` lands on „Deckkraft“ and at `…panel.color` on „Farbe“. `HeroColorField` gained a `tokens` prop for the two vocabularies, the Panel's `black` being one no text colour knows
-   The Kopfbereich of the portal is editable: the instance's Portal tab leads with the required „Portalname“ and opens the Hero Editor, which places up to twelve Blocks in the nine Zones over a „Hintergrund“ and renders every change in two storefront frames. „Auf Standard zurücksetzen“ returns it to the layout derived from Portalname and logo. The entries below carry the parts: the editor's shell, Blocks, „Hintergrund“ and „Live-Vorschau“, the Mediathek's „Kopfbereich“ usage, and the `Tiptap.vue` props `links` and `maxLength` the formatted-text Blocks needed
-   Hero Editor: the Draft carries the amended Block shape before the form offers a control for it. `panel` is `null` or the object `{ color, opacity, radius, blur }` whose four defaults are the „Glas“ preset (`white`, `60`, `md`, `blur`), the legacy strings `"none"` and `"translucent"` being read into it on the way in; `align` (`auto | left | center | right`) joins the common fields of every Block type, `offset` becomes `{ x, y }` in rem, `layer` is `back | front`, and rich-text Blocks carry `size` like text Blocks do. A layout that already has them loads, is selected, moved, duplicated and saved with them untouched — a duplicate keeps the Panel, alignment, Offset and layer and takes its own id — and a layout that has none of them is completed on the way in, so the Draft the preview round-trip receives is whole and the backend fills nothing in behind the editor's back. The derived default of „Auf Standard zurücksetzen“ goes through the same completion. `normalizeHeroBlock` and `normalizeHeroLayout` are where it happens; the one existing Panel switch now writes the „Glas“ object and `null` instead of the legacy strings, and the form gains no other control yet
-   Rich-text editor (`Tiptap.vue`): opt-in props `sizes`, `colors`, `paragraphAlign` and `themeColors` for the Hero Editor's „Formatierter Text“ Blocks. With them the leiste splits into the two captioned rows „Zeichen“ — Fett, Kursiv, Unterstrichen, Link, „Schriftgröße“ as a visible seven-step scale and „Farbe“ as four colour dots, the two token dots painted with the instance's own `branding.theme.colors`, plus „Eigene Farbe…“ and „Farbe übernehmen“ — and „Absatz“ — „Ausrichtung“ as a four-state segment and the two list buttons. „Übernehmen“ leads every group and is a value, not a toggle: it is the absence of the class, and the Block's own step is written out as soon as the author picks it, so a word set to „Normal“ inside a „Normal“ Block keeps that choice over a reload. Size and colour are one mark, so a marked run of words is a single `<span class="hero-size-lg hero-color-primary">`; a custom hex renders `data-color` and a `style="color:#hex"` the admin alone carries, so the colour is visible while typing. The props switch controls, not schemas — the mark loads as soon as either `sizes` or `colors` is set, so a stored `hero-color-*` is never lost. On load the editor normalises onto the vocabulary of `src/utils/heroRichtextClasses.js`: a class off the list parses to „Übernehmen“, the first of two competing classes wins, a token beats a `data-color`, a `data-color` that is not `#rrggbb` is dropped, and a `<span>` with nothing recognised is unwrapped, keeping its text. That is a convenience, **not** a security boundary — the boundary stays the sanitisers on save and on render. Without the new props the four existing users are unchanged, pinned by the characterisation spec
-   Hero Editor: the language toggle and the backend's field errors. „Deutsch | English“ switches every localised field — text, rich text and „Alternativtext“ — to the other locale; an empty English field shows the German text as its placeholder with the hint „Leer: Deutsch wird angezeigt“, required marking stays German-only, and a badge „N ohne Übersetzung“ beside the toggle counts the Blocks the storefront would show in German on the English page. A `400` of the preview round-trip or of „Speichern“ is read out of `details[].field`, whose JSON path resolves to Block, field and locale: the message stands under the control it is about and marks the Block's row red, a fault of the layout or of the Background at its section, and a path the editor cannot place above the form rather than nowhere. „Speichern“ stays disabled while the last round-trip was refused and is released by the next accepted Draft; a refused save keeps the dialog open with its fields marked. A message stands at the deepest field the editor can name, so a fault of a media reference lands on „Bild“ rather than at the section. The path reading lives in `src/utils/heroErrors.js`
-   Hero Editor: the „Live-Vorschau“ answers back. A Block click in a frame selects that Block in the list and sends the Draft down again with the selection, so the frame highlights it and shows its Zone overlay; a Zone click moves the selected Block to the end of that Zone's stack, clicking its own Zone doing nothing — the storefront reports the click and never moves anything itself. Every rendered Draft is answered with a `hero-preview:report` per frame, matched to the current `draftId` so a superseded one is discarded: its warnings become a yellow badge on the Block's row („Ragt auf dem Desktop aus dem Kopfbereich heraus“, „Überlappt auf dem Desktop mit ‚…‘“ naming the other Block by its first line, „Wird auf Mobilgeräten abgeschnitten“) and a count „N Hinweise“ beside each frame. A report carrying `invalid-draft` is logged and leaves the last render and its badges standing. Warnings never hold a save back; the wording lives in `src/utils/heroPreviewReport.js`
-   Hero Editor: the „Live-Vorschau“ — two storefront preview frames, desktop at a virtual 1280 px scaled to the panel and mobile at 375 px, stacked below 1100 px, with a toolbar for „Startseite | Unterseite“ (the frame's `mode=home|compact`), „Hell | Dunkel“ and a reload; the locale comes from the header toggle. Every Draft change is debounced ~300 ms through `POST /api/catalog/hero-layout/preview` and only a 200 is posted to both frames as `hero-preview:draft` (protocol 1); a 400 leaves the last valid preview standing and disables „Speichern“ until the next 200, and a locally invalid Draft is not sent at all. Posting and message filtering always use the origin of the Portal-URL, never `*`; without one the panel shows „Portal-URL fehlt“ and the editor still saves, and without a `hero-preview:ready` within 10 s it shows „Vorschau nicht erreichbar“ with a reload button. The storefront owns the protocol; the copy lives in `src/utils/heroPreviewProtocol.js`
-   Hero Editor: the „Hintergrund“ section — the three family cards „Muster | Farbe | Bild“, which remember the last values of the families they switch away from for as long as the dialog is open, and „Standardhintergrund“, the Background's own reset to today's `poly` pattern. „Muster“ edits `variant`, „Leuchtkreise“, „Körnung“ and „Intensität“; „Farbe“ a hex colour and an optional one for the dark mode, whose key goes away when the field is cleared; „Bild“ a public image of the instance's Mediathek, its focal point by clicking the thumbnail and the overlay as a colour plus the slider „Abdunklung“, optionally with own values in the dark mode. The Background is saved beside the layout and never touches the „Standard-Layout“ chip, which is about the layout alone; local pre-validation mirrors the backend — hex format, an image required, whole percentages — and shows what it refuses at the section while „Speichern“ stays disabled
-   Hero Editor: the „Inhalt“ section of rich-text and image Blocks. „Formatierter Text“ edits its HTML in the shared `Tiptap.vue` with `links` and a 10 000-character counter, one instance per locale view; „Bild“ picks a public image of the instance's Mediathek, shows the thumbnail and edits „Alternativtext“, „Maximale Höhe“ and „Im Dunkelmodus invertieren“. Enriched references round-trip untouched. Local pre-validation now covers all three types
-   Hero Editor: the detail form of the selected Block — „Inhalt“ → „Position“ → „Darstellung“ → „Sichtbarkeit“. A text Block edits its text in the locale the header toggle selects, with a 200-character counter, „Schriftgröße“, the switches „Fett“ and „Schatten für bessere Lesbarkeit“ and the colour control: the chips „Standard“, „Primärfarbe“, „Sekundärfarbe“ (painted with the instance's `branding.theme.colors`), „Weiß“ and „Eigene…“, the last opening the hex picker without alpha. „Position“ is a 3×3 grid of the nine Zones that counts the other Blocks per cell and moves the Block to the end of the chosen Zone's stack; „Darstellung“ carries `outerSpacing`, `innerSpacing`, `width` and the translucent panel, „Sichtbarkeit“ `homeOnly` and `hideOnMobile`. Local pre-validation mirrors the backend — German text required, at most 200 characters, a valid hex on a custom colour — and an invalid Block marks its row and disables „Speichern“. Rich text and images follow
-   Hero Editor: the „Blöcke“ section — Blocks grouped by Zone in Zone order („Oben links“ … „Unten rechts“, empty groups hidden), a row per Block with type icon, first German line, the icons for `homeOnly` and `hideOnMobile` and a slot for the coming warning badges. A row is selected by clicking it; its ⋮ menu moves it inside the group, duplicates it behind the original with a fresh uuid v4 or deletes it without asking; drag by handle reorders inside a group and moves across groups, setting the Zone at the drop position. „Block hinzufügen“ offers Text / Formatierter Text / Bild, places the new Block behind the selected one or in „Mitte zentriert“ and is disabled at twelve Blocks. After every change the array is rewritten in canonical order (Zone order, then in-group order), which fixes the mobile reading order to left → centre → right. The rules live in `src/utils/heroBlocks.js`
-   Hero Editor: the full-screen dialog behind „Kopfbereich bearbeiten“ with its own load/save cycle over `GET`/`PUT /api/catalog/hero-layout` and `POST /api/catalog/hero-layout/preview` — skeleton while loading, a status chip „Standard-Layout (folgt Portalname und Logo)“ or „Angepasst“, the language toggle, the three „Höhe“ selects (Startseite, Mobilgeräte, Unterseiten) and „Auf Standard zurücksetzen“. While the layout is the default one the save sends `heroLayout: null`, so it keeps following Portalname and logo; „Abbrechen“ with changes asks before it drops them and a page unload is guarded while the draft is dirty. Blocks, Hintergrund and the Live-Vorschau follow
-   Media library: usage type „Kopfbereich“ (`hero`) — an image Block of the instance catalog's Hero Layout or the Background image — with icon `mdi-page-layout-header` in the usage list and in the blocked dialog, linking to the Portal tab of the instance editor for instance admins. A visibility downgrade to „intern“ that the backend refuses as in use now names the blocking usages in that dialog instead of a generic error toast, and leaves the form open. The dialog now links every usage it lists, not only the Kopfbereich
...
-   Media picker: new prop `allowExternal` (default true) on `MediaReferenceField` and `MediaPickerDialog` — a reference site the backend only accepts as a medium of the library hides the „Externer Link“ tab. The Hero Editor's image Blocks are the first such site; the other five call sites are unchanged
