...
-   `docs/agents/booking-status-vocabulary.md` (spec §2): a section on the headline over the path - Kopfzeile, Hauptpfad, Schritt/Segment, Endsegment, Erreicht, Hauptaktion and Nebenweg ("always in the ⋮-menu") against `bookingStatus.js` and `BookingStatusPath.vue`; the Anfangszustand addendum to E10 with the segment → choice → wire table; the series rules (Kostenfrei = total price 0, Erreicht by every member, the shared reason, Gemischt counts, the series verbs). `docs/agents/access-vocabulary.md`: the row "Vom Anbieter übernehmen / Manuell anlegen" → `mode`, and the Herkunft row says a taken-over door still reads "Selbst angelegt"
-   Access point management (spec N7): one button "Zugangspunkt anlegen" over the table replaces "Tür anlegen" and "Aus Anbieter übernehmen"; the dialog of the same name opens on a switch "Vom Anbieter übernehmen" / "Manuell anlegen", shown only while a provider is active and preset to the provider, and switching keeps what was entered. Without a provider the dialog is the door form, without the hint "Kein Anbieter aktiv". Editing is unchanged. The dialog's `source` prop is its state `mode` now; the strings `createDoor`, `createFromProvider`, `createDoorTitle`, `createFromProviderTitle` and `picker.noProvider` are gone
...
-   Bookable editor, tab "Schließsysteme": one assignment table for doors and locker systems instead of the provider cards and the unit list. A locker system is assigned by id in `accessPointDetails.accessPointIds` like a door, with the access buffer, a picker over the tenant's access points, and a column saying what a booking gets. The bookable's `amount` sits above the table as the capacity, locked while an external provider reports it (`utils/bookableExternalProviders.js`). The old view iterated `city.locations` on the facade's flat 4.3.x answer and threw a TypeError. "Schließfach" reads "Anlage" now, in the access point management too
-   Bookable editor, tab "Schließsysteme": a column "Menge" distributes the compartments a booking gets over the assigned locker systems, stored in the new `accessPointDetails.accessPointAmounts` (`{ "<accessPointId>": <n> }`); a system left empty keeps one compartment per booked unit. An amount goes with its assignment when the access point is removed. `amount` stays editable — where the distribution does not add up to it the tab warns and saves anyway, a deliberate decision: the compartments follow the distribution while occupancy is still counted against `amount`. Reader: `utilities/access-point-amounts.js`
