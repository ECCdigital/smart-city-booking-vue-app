...
-   Access point management (spec N7): one button "Zugangspunkt anlegen" over the table replaces "Tür anlegen" and "Aus Anbieter übernehmen". The dialog, titled "Zugangspunkt anlegen", opens on a segment switch "Vom Anbieter übernehmen" / "Manuell anlegen" with a caption each - shown only while a provider is active, preset to the provider; "Manuell" hides the picker row and keeps what was entered. Without a provider the dialog is the door form, and the hint "Kein Anbieter aktiv" is gone. Editing is unchanged. The dialog's `source` prop becomes its state `mode`, `openCreate()` takes no argument; new strings `accessPoint.management.create` and `dialog.createTitle/modes/modeHints`, `createDoor`, `createFromProvider`, `createDoorTitle`, `createFromProviderTitle` and `picker.noProvider` are gone
