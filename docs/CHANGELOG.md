...
-   `docs/agents/access-vocabulary.md`: the glossary of the access area — the German UI terms (Anlage, Fach, Vorgemerkt/Erteilt/Widerrufen, Menge, Stückzahl, Reichweite) against the code identifiers they map to, why „Locker“ stays out of the copy while `locker` stays the type value, the opaque Fach-Id that is never split or assembled, and the one source a booking's Zugänge are read from. Listed in `AGENTS.md` and `docs/agents/README.md`
...
-   Bookable pricing, external provider: the iFBS price panel and the price rows on the bookable card read the 4.3.x answer, a flat array of `{priceEur, unit, external}`. They used to index it as an object by the provider's own German keys (`Preis_1h`, `Preis_Servicegebühr`), so **every figure they showed was `undefined`**. A rate the provider does not charge is no longer a `0,00 €` tile: the answer leaves it out, and so does the panel. Both read the prices from `GET /:tenant/bookables/:id/prices`, the categories the checkout itself prices with, instead of the `/locker/:provider/locations/:id/price` facade — with it the last `/locker/*` caller leaves the UI and `ApiLockerService` is gone. Two figures fall with the facade, deliberately: the location's **Mindestnutzungsdauer** is consumed inside the backend's duration check and is delivered by no 4.3.x route, and **Fahrradboxen gesamt / Pufferzeit** came from the provider's statistics record, which `/locker/:provider/status` no longer answers with — it answers with the listed access point. Asking the backend to re-expose them would be a PR for two read-only tiles, which the strand's narrow rule does not carry; the capacity is the bookable's `amount` in the access tab, and the buffer applies at checkout. What makes the panel apply is now the assigned locker system — an access point of the provider, read from `accessPointDetails.accessPointIds` — instead of the derived `bookable.lockerDetails`, which no longer has a reader in the UI. The pricing tab's section nav keys the external-source anchor on the declared provider, since a bookable's ids alone do not say which of its access points is a locker system. The prices route is a public one — it answers only for a stored, publicly visible bookable — so neither screen asks for a draft or an unlisted bookable, and the editor names that as the reason instead of reporting a refusal as a provider failure. The array reader lives in `utils/externalPriceCategories.js`, the reading of the route in `mixins/externalPrices.js`; `BookableCard.vue` also loses an unused `defaultImage`

-   Booking details: one list "Zugänge" for doors and compartments instead of a door section beside a separate "Schließfach-Steuerung" that only showed confirmed iFBS boxes. Its only source is `GET /:tenant/access?bookingId=` — the tenant-scaled projection, the one a tenant owner reaches for a booking that is not their own; `booking.lockerInfo` and `booking.accessInfo` are no longer read. A row names its access point, at a compartment the compartment ("— Fach 17"), the provider and the provider's process id, and carries a state chip — Vorgemerkt / Erteilt / Widerrufen, read off `externalBookingId` and `isProvisioned`. The open command is addressed with the `id` the projection hands out, unsplit and unassembled: the old locker section sent `lockerInfo.processId` as the id, which since 4.3.x is neither the access point id nor the opaque compartment id (`<accessPointId>:<authorizationId>`), so **every open command answered 403**. The open button now stays visible and is disabled with its reason on the element where it cannot be operated, rather than disappearing. The list lives in its own component `BookingAccessPoints.vue` over `utilities/booking-access-points.js`. Two things fixed on the way: an entry whose window the projection leaves `null` no longer reads as expired for good (`null >= now` is false, so its button died and the hint said "bis  abgelaufen"), and a booking whose accesses cannot be read says so instead of looking as if it had none. The messages doors and compartments now share no longer speak of "Tür" — an iFBS error code keeps its own German text

-   Booking details, access messages: no 403 is read as "outside the time window" any more. `open`/`unlatch` answer 403 only for an access point that is not part of the booking, `close`, `/status` and `/open-status` answer it whenever the booking is not operable at all — each caller names its reading, the default is gone. A provider failure at `open` arrives as HTTP 200 `{success:false, data:{openFailure}}` and is now named as a configuration error or a temporary fault instead of "unbekannter Grund". The open button is freed by `capabilities` as well as `mode` — a Pareva system declares no `open` and no longer renders a live button — and the reason at a blocked button is read off the projection's own fields: no remote access, closed window, missing or withdrawn grant, evidence this screen cannot send. A started open is followed through `/open-status` with the `openProcessId` its answer names, instead of `/status`, which a locker system without `getStatus` answered "unknown" eight times over before reporting a timeout; an open that names no process was already carried out and is not polled at all. A poll that could not tell (`confirmed: null`) no longer reads as "not confirmed yet" and says so with the provider's own reason where it has one. A 403 that carries a `ForbiddenError` body came from the authorization middleware and is named as the denial it is, whichever route answered — a route's own refusal is a bare 403. "Schließen" is only offered where the provider declares `close`, which retires the hard-coded Salto KS exception, and "Status prüfen" is blocked with its reason where the provider declares no `getStatus` — the button stays where it is, as the open button does. `locker_not_ready` is gone from the blocking reasons — the backend dropped it from its enum

-   Access point management: one table for doors and locker systems — new columns Typ („Tür“ / „Anlage“) and Herkunft („Selbst angelegt“ / „Vom Anbieter“), a type filter over the table, „entfällt“ in the QR column of an Anlage (it has no validation rules, so its QR and scan-code entries are gone from the row menu too) and a note in its assignment cell that the capacity is set on the bookable. Two buttons — „Tür anlegen“ and „Aus Anbieter übernehmen“ — open the one edit dialog in its two starting states; there the type follows the provider and is only shown, not chosen, and mode, QR rules and address are hidden for an Anlage, which is saved with `validationRules: []` instead of the server's `qrScan` default. The delete dialog additionally names the running bookings that hold a granted, unrevoked access — the deletion drops the row without revoking anything at the provider — read from the tenant's bookings (`accessInfo`), the only source a tenant owner can reach: `GET /:tenant/access` answers per booking, and `/access/access-points/:id/bookings` reaches only the caller's own
-   Tenant applications: Pareva and Parkraumservice are written and read as `type: "access"`, the only type the 4.3.x schema knows for them. The UI wrote `"locker"`, which is out of the schema — the backend hands such an entry back untouched but its provider layer does not see it. Reading is a hard cut too: an application still carrying `"locker"` does not count as stored, so a tenant whose applications were not migrated reads as not configured and is set up again, instead of showing a configuration that the next save would write the dead type back into. No fallback, no warning. The connection test of both providers moved from the `/locker/:provider/test` facade to `/access-apps/:provider/test` — the same provider test handler, the same answer body, but `accessApp.manage` where the facade asked `manageBookables.readAny`: the test is the tenant owner's now
-   Bookable editor, tab "Schließsysteme": one assignment table for doors and locker systems instead of the three provider cards and the unit list. A locker system is an access point since 4.3.x, so it is assigned by id in `accessPointDetails.accessPointIds` like a door — with the access buffer, a picker over the tenant's access points, and a column saying what a booking gets: a door is shared for the period, a locker system hands out one compartment per booked unit. The bookable's `amount` sits above the table as the capacity those compartments are counted against, editable here as well, and locked while an external provider reports it — the rule the pricing tab already applied, now shared as `utils/bookableExternalProviders.js`. The old view read the `/locker/:provider/locations` facade, whose 4.3.x answer is a flat list, and iterated `city.locations` on it: the section threw a TypeError against 4.3.x. The "provider not active in the tenant" signal is gone with the cards — the picker lists what the tenant has. The type of a locker access point now reads "Anlage" instead of "Schließfach", in the access point management screen too. The characterisation test for the tenant-app-list reading was replaced deliberately: the reading it pinned no longer exists
-   Bookable editor, tab "Schließsysteme": the assignment table gained a column "Menge" — how many compartments a booking gets at each assigned locker system, stored in the new `accessPointDetails.accessPointAmounts` (`{ "<accessPointId>": <n> }`) beside the unchanged flat `accessPointIds`, so a door notices nothing of it. The backend reads the field since `accessPointAmounts`; a system the column leaves empty keeps what it had, one compartment per booked unit. An amount goes with its assignment when the access point is removed, and one for an access point the bookable does not reference is dropped before the PUT. `amount` stays freely editable — where the distributed amounts do not add up to it the tab warns and saves anyway, a deliberate decision with a known risk: the compartments follow the distribution while the occupancy is still counted against `amount`. A bookable that distributes nothing at all draws no warning: it behaves as it always did, so the sum is not yet a statement to compare. The reader is `utilities/access-point-amounts.js`
