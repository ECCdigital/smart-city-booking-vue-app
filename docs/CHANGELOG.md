...
-   Booking detail drawer (spec N2, N3, N4): the status bar becomes a headline over the booking's path. An avatar and the state word in the state's colour, the Kostenfrei marker, the primary transition as a button (Freigeben, Als bezahlt markieren, Wiederherstellen) and the side ways (Ablehnen / Stornieren) in a ⋮-menu; under it a segmented line Angefragt · Zahlung offen · Bestätigt (without Zahlung offen for a free booking) with the request date and "bezahlt {date}" at Bestätigt, cut by a red end segment with the cancellation date at Abgelehnt / Storniert. The reason moves out of "Buchungsinformationen" into a block under the path, captioned Ablehnungsgrund or Stornierungsgrund by state. The path view lives in `bookingStatus.js` (`pathOf`, `splitActions`, `STEP_STATE`), the headline in `BookingStatusPath.vue`; the edit form and the series drawer still render `BookingStatusBar`. New string `booking.status.paidShort`; `BookingDetails.spec.js` ("the state", "the actions") was rewritten deliberately
