...
-   Booking edit form (spec N4): the status section renders the same headline over the path as the detail drawer — state word, Kostenfrei marker, the primary transition as a button and the side ways in the ⋮-menu, the segments with their dates. Button and menu stay locked while the form has unsaved changes, with "Erst speichern" under the path. At Abgelehnt / Storniert the reason is edited in the block under the path (caption by state, an outlined textarea, still required); the accent sheet is gone and the refund audit is a sheet of its own below. The section reports the reason as `update:rejection-reason`, the form writes the booking. `BookingEditStatus.spec.js` was rewritten deliberately; the create mode is untouched
