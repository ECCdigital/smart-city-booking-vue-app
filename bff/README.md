...
-   A failed CSRF check answers **`419`** (`{ success: false, message: "CSRF check failed" }`), not `403` — a `403` is the backend's own permission denial and the Admin UI shows a different message for each. Clients that check for `403` here need updating (`src/csrf.js`, mirrored as `CSRF_FAILED_STATUS` in the UI's `src/services/api/apiErrorMessage.js`)
